...
- (void)uploadStudentMsg{
    ...
    [request setPostValue:@"StudentCheck" forKey:@"action"];
    [request setPostValue:userInfo[@"coachid"] forKey:@"coachid"];
    [request setPostValue:userInfo[@"token"] forKey:@"token"];
    [request setPostValue:_studentId forKey:@"studentid"];
    
    //5.判断相关证件是否为空
    //身份证正面
    [request setData:UIImageJPEGRepresentation(self.idCardImageView.image, 0.75) forKey:@"idcardf"];//身份证正面照
    
    //身份证反面
    [request setData:UIImageJPEGRepresentation(self.idCardBackImageView.image, 0.75) forKey:@"idcardb"];//身份证反面照
    
    //驾照或者学员证
    [request setData:UIImageJPEGRepresentation(self.cardImageView.image, 0.75) forKey:@"cardpic"];
