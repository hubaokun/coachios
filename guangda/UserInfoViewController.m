...
- (void)changeUserMsg{
    ...
    [request setPostValue:name forKey:@"realname"]; // 密码
    [request setPostValue:userInfo[@"coachid"] forKey:@"coachid"]; // id
...
- (void)requestFinished:(ASIHTTPRequest *)request {
    ...
    if ([code intValue] == 1) {
        ...
        MyInfoCell *curCell = _cells[0];
        NSString *name = curCell.contentField.text;
        [ds setObject:name forKey:@"realname"];
        curCell = _cells[1];
        NSString *phone = curCell.contentField.text;
        [ds setObject:phone forKey:@"phone"];
